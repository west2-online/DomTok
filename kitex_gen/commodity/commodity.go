...
type ListSkuInfoReq struct {
	SkuVersions []*model.SkuVersion `thrift:"skuVersions,1,required" frugal:"1,required,list<model.SkuVersion>" json:"skuVersions"`
...
func (p *ListSkuInfoReq) GetSkuVersions() (v []*model.SkuVersion) {
	return p.SkuVersions
}
...
func (p *ListSkuInfoReq) SetSkuVersions(val []*model.SkuVersion) {
	p.SkuVersions = val
}
...
func (p *ListSkuInfoReq) DeepEqual(ano *ListSkuInfoReq) bool {
	...
	if !p.Field1DeepEqual(ano.SkuVersions) {
		return false
	}
	if !p.Field2DeepEqual(ano.PageNum) {
		return false
...
func (p *ListSkuInfoReq) Field1DeepEqual(src []*model.SkuVersion) bool {
	...
	if len(p.SkuVersions) != len(src) {
		return false
	}
	for i, v := range p.SkuVersions {
		_src := src[i]
		if !v.DeepEqual(_src) {
			return false
		}
